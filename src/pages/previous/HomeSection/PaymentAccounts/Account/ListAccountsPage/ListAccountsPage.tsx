...
import { Modal, ModalFooter, ModalHead } from "@/components/previous/all/Modal";
...
const ListAccountsPage = () => {
  ...
  const feachingAccount = accounts?.data?.map((account: any) => {
    const findBranch = branches?.data?.find(
      (branch: any) => branch?.id === account?.branchId
    );
    return {
      ...account,
      branch: findBranch,
    ...
    <div>
      <div>
        <h1 className="my-3 ml-4 ">
          <span className="text-3xl font-bold">Accounts List</span>{" "}
      ...
      <div className="w-[98%] mx-auto bg-slate-100 border-t-2 border-brand pb-10 shadow-xl rounded">
        ...
        <div className="mt-5 mx-12">
                      ...
                      <Tbrow key={account?.id}>
                        ...
                        <Td>
                          <div className="flex items-center gap-3">
                            <div>
                              <Link to={`/edit_account/${account?.id}`}>
                                <Button
                            ...
                            <div>
                              <Button
                                handleClick={() => deleteAccount(account?.id)}
                                bgColor="bg-red-500"
          ...
          <hr className="m-5" />
          {feachingAccount?.length > 5 && (
            <div className="mt-5 mr-8 flex justify-end">
              <Pagination
                currPage={currentPage}
                setCurrPage={setCurrentPage}
                isLoading={null}
                totalItems={feachingAccount?.length}
                totalPage={Math.ceil(feachingAccount?.length / 5)}
                pageLength={5}
              />
            </div>
          )}
