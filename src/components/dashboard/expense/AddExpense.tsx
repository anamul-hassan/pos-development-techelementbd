...
import { useAddExpenseMutation } from "@/store/expense/expenseApi";
...
interface IAddExpenseProps {
  setAddExpenseOpen: (addExpenseOpen: boolean) => void;
}

const AddExpense: FC<IAddExpenseProps> = ({ setAddExpenseOpen }) => {
  // DATE STATE
    ...
    <form onSubmit={handleSubmit(handleAddExpense)}>
      ...
      <div className="flex justify-between items-center my-2 w-full">
        <div className="flex justify-start w-full md:max-w-[300px]">
          {addExpenseError &&
            Object?.keys(addExpenseError)?.length > 0 &&
            "data" in addExpenseError && (
              <Alert variant="destructive">
                <AlertCircle className="h-4 w-4" />
                <AlertTitle>Add Expense Error</AlertTitle>
                <AlertDescription>
                  {addExpenseError?.data?.message ||
        ...
        <div className="flex justify-end w-1/2">
          <Button disabled={addExpenseLoading} type="submit">
            {addExpenseLoading && <ButtonLoader />}
            Add Now
          </Button>
        </div>
      </div>
